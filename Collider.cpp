...
void Collider::EnemyMove(Collider& other, int* direction) {
	...
	{
		if (intersectX > intersectY) {	// Y의 차보다 X의 차가 크다면
			if (deltaX > 0.0f) {	// 오른쪽 벽에 충돌
...
bool Collider::CheckCollision(Collider& other, Vector2f& direction, float push) {
	...
	if (intersectX < 0.0f && intersectY < 0.0f)
	{
		...
		if (intersectX > intersectY) {	// Y의 차보다 X의 차가 크다면
			if (deltaX > 0.0f) {	// 왼쪽에서 충돌
				Move(intersectX * (1.0f - push), 0.0f);	// 0이면 BLOCK을 해당 방향으로 밀기
				if(intersectY!=-GRAVITY)
					other.Move(-intersectX * push, 0.0f);	// X로는 계속 움직이고, Y를 고정
			}
			else {	// 오른쪽에서 충돌
